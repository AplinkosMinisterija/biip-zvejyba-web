...
const FishingActions = ({ fishing }: FishingActionsProps) => {
  ...
  const warnedAboutUnchecked = useRef(false);
  ...
  return loading ? (
    ...
    <>
      <NotCheckedToolsPopup
        locations={notCheckedLocations}
        onClose={() => setNotCheckedLocations([])}
      />
      <Container>
        <LargeButton
