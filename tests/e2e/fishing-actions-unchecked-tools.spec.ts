...
test.describe('Unchecked tools warning before shore weighing and ending the fishing', () => {
  ...
  test('"Baigti" warns first, then asks for the end confirmation', async ({ page }) => {
    ...
    await expect(page.getByText(texts.notCheckedTitle)).toBeVisible();
    await expect(page.getByText(texts.barName)).toBeVisible();
    await expect(page.getByText(texts.endFishingTitle)).toHaveCount(0);
  ...
  test('"Baigti" decides on fresh data, not the payload cached before the checks', async ({
    page,
  }) => {
    await mockFishing(
      page,
      { preliminary: {}, unfinishedCheckLocations: unfinished },
      { preliminary: {}, unfinishedCheckLocations: [] },
    );
    await page.goto(CURRENT_FISHING_PATH);
  ...
  test('"Baigti" goes straight to the end confirmation when nothing is left unchecked', async ({
    page,
  }) => {
    await mockFishing(page, { preliminary: {}, unfinishedCheckLocations: [] });
    await page.goto(CURRENT_FISHING_PATH);
